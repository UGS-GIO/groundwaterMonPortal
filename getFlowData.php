<?php 
	...
	else if($type == "monthly")
	{
		...
		$sql = "SELECT CAST(EXTRACT(MONTH FROM flowdate) as varchar) || '/' || CAST(EXTRACT(YEAR FROM flowdate) AS varchar) as FlowDate, CAST(EXTRACT(YEAR FROM flowdate) AS varchar) || '-' || RIGHT ('00' || CAST(EXTRACT(MONTH FROM flowdate) AS varchar), 2) as dtOrder, TRUNC(Sum(Discharge) / COUNT(CAST(EXTRACT(YEAR FROM flowdate) AS varchar) || '-' || RIGHT ('00' || CAST(EXTRACT(MONTH FROM flowdate) AS varchar), 2)),4) as Discharge from UGS_GW_FLOW where LOCATIONID IN (" . implode(',', $ph) . ") group by CAST(EXTRACT(YEAR FROM flowdate) AS varchar) || '-' || RIGHT('00' || CAST(EXTRACT(MONTH FROM flowdate) AS varchar), 2), CAST(EXTRACT(MONTH FROM flowdate) AS varchar) || '/' || CAST(EXTRACT(YEAR FROM flowdate) AS varchar) order by dtOrder;";
	}
	else
	{
		die("Invalid type provided.");
