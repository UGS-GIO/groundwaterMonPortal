CREATE OR REPLACE FUNCTION gwportal.get_groundwater_data_plus_manual_data(
   p_well_id TEXT,
   p_type TEXT,
   p_from_date DATE,
   p_to_date DATE
   ...
   IF lower(v_location_type) != 'spring' THEN
       IF p_type = 'daily' THEN
           v_sql := '
               SELECT 
                   ...
                   AVG(m.waterelevation)::TEXT as manual_elevation
               FROM gwportal.reading r
               LEFT JOIN gwportal.ugs_gw_manualdata m ON m.locationid = r.locationid 
                   AND date_trunc(''day'', m.readingdate) = date_trunc(''day'', r.readingdate)
       ...
       ELSIF p_type = 'monthly' THEN
           v_sql := '
               ...
               FROM gwportal.reading r
               LEFT JOIN gwportal.ugs_gw_manualdata m ON m.locationid = r.locationid 
                   AND date_trunc(''month'', m.readingdate) = date_trunc(''month'', r.readingdate)
               WHERE r.locationid = $1
                 AND r.readingdate BETWEEN $2 AND $3
   ...
   ELSE
       IF p_type = 'daily' THEN
           v_sql := '
               ...
               LEFT JOIN gwportal.ugs_gw_manualdata m ON m.locationid = f.locationid 
                   AND date_trunc(''day'', m.readingdate) = date_trunc(''day'', f.flowdate)
       ...
       ELSIF p_type = 'monthly' THEN
           v_sql := '
               ...
               LEFT JOIN gwportal.ugs_gw_comments c ON c.comment_id = f.comments
               LEFT JOIN gwportal.ugs_gw_manualdata m ON m.locationid = f.locationid 
                   AND date_trunc(''month'', m.readingdate) = date_trunc(''month'', f.flowdate)
               WHERE f.locationid = $1
                 AND f.flowdate BETWEEN $2 AND $3
